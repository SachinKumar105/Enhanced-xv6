...
{   
  ...
  31:	6a 24                	push   $0x24
  33:	e8 78 06 00 00       	call   6b0 <malloc>
  ...
  45:	50                   	push   %eax
  46:	56                   	push   %esi
  47:	e8 4e 03 00 00       	call   39a <getpinfo>
    printf(1,"proc stat:\n pid = %d \t current_queue = %d \t num_run = %d\n",ps->pid,ps->current_queue,ps->num_run);
  4c:	5b                   	pop    %ebx
  4d:	ff 76 08             	pushl  0x8(%esi)
  50:	ff 76 0c             	pushl  0xc(%esi)
    for(int i=0;i<5;i++)
  53:	31 db                	xor    %ebx,%ebx
    printf(1,"proc stat:\n pid = %d \t current_queue = %d \t num_run = %d\n",ps->pid,ps->current_queue,ps->num_run);
  55:	ff 36                	pushl  (%esi)
  57:	68 d0 07 00 00       	push   $0x7d0
  5c:	6a 01                	push   $0x1
  5e:	e8 ed 03 00 00       	call   450 <printf>
  63:	83 c4 20             	add    $0x20,%esp
        printf(1,"ticks[%d] = %d\n",i,ps->ticks[i]);
  66:	ff 74 9e 10          	pushl  0x10(%esi,%ebx,4)
  6a:	53                   	push   %ebx
    for(int i=0;i<5;i++)
  6b:	83 c3 01             	add    $0x1,%ebx
        printf(1,"ticks[%d] = %d\n",i,ps->ticks[i]);
  6e:	68 bf 07 00 00       	push   $0x7bf
  73:	6a 01                	push   $0x1
  75:	e8 d6 03 00 00       	call   450 <printf>
    for(int i=0;i<5;i++)
  7a:	83 c4 10             	add    $0x10,%esp
  7d:	83 fb 05             	cmp    $0x5,%ebx
  80:	75 e4                	jne    66 <main+0x66>
    exit();
  82:	e8 5b 02 00 00       	call   2e2 <exit>
  87:	66 90                	xchg   %ax,%ax
  89:	66 90                	xchg   %ax,%ax
  8b:	66 90                	xchg   %ax,%ax
  8d:	66 90                	xchg   %ax,%ax
...
printf(int fd, const char *fmt, ...)
{
 ...
 604:	e9 8f fe ff ff       	jmp    498 <printf+0x48>
          s = "(null)";
 609:	bb 0c 08 00 00       	mov    $0x80c,%ebx
        while(*s != 0){
