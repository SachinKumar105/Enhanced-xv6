...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      p->state = RUNNING;
      p->num_run++;
      // cprintf("process %s started with pid %d running at %d\n",p->name, p->pid,p->ctime);
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p!=0)
      {
        ...
        p->state = RUNNING;
        p->num_run++;
        // cprintf("process %s started with pid %d and ctime %d running at %d\n",p->name, p->pid,p->ctime,ticks);
        swtch(&(c->scheduler), p->context);
        switchkvm();
        c->proc = 0;
      }
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p!=0)
      {
        ...
        p->state = RUNNING;
        p->num_run++;
        // cprintf("process %s started with pid %d, ctime %d and priority %d running at %d\n",p->name, p->pid,p->ctime,p->priority,ticks);
        swtch(&(c->scheduler), p->context);
        switchkvm();
        c->proc = 0;
      }
  ...
  for(;;){
    ...
    { if(q_count[i]==0)
      ...
      for(p = p; p < &ptable.proc[NPROC]; p++){
        ...
        // if(p->queue_num == i && p->state == RUNNING)
        // {
        //   p->queue_num++;
        // }
        // Process is done running for now.
...
updatetime()
{
  ...
  { 
    ...
    { p->wtime++;
      ...
      if(p->last_run_time > 30)
      {
        if(p->queue_num>0)
        { p->queue_num--;
          cprintf("pid %d on cpu - %d going up to %d \n",p->pid,cpuid(),p->queue_num);
        }
          p->time_quantums = 1 << (p->queue_num);
      }
    #endif // MLFQ
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        *wt = p->wtime;
        *rt = p->runtime;
        // cprintf("values are for %s with pid %d :%d %d \n",p->name,p->pid,p->wtime,p->rtime);
        pid = p->pid;
        kfree(p->kstack);
        p->kstack = 0;
        freevm(p->pgdir);
